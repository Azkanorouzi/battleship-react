...
export default function BackgroundSplitDecor() {
  ...
  return (
    ...
    <motion.div className={`w-[50vw] lg:h-screen -z-20 transition-colors ${gameData.turn === 'user' ? 'bg-secondary' : 'bg-slate-900'}`} initial={{opacity: 0}} animate={{opacity:1}} transition={{duration: .8, delay: 1}}></motion.div>
    </div>
  )
}
